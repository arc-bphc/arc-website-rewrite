...
export default function ASCIIText({
  ...
  useEffect(() => {
    ...
    // This function will handle the async setup
    const init = async () => {
      if (!containerRef.current) return;

      // --- FIX: Wait for the font to be loaded and ready ---
      // The font string must match exactly what you use in the canvas context
      // (fontWeight fontSize fontFamily)
      const font = `600 ${textFontSize}px "Zen Dots"`;
      await document.fonts.load(font);

      // Now that the font is loaded, we can safely initialize our scene
      const { width, height } = containerRef.current.getBoundingClientRect();

      // Create the ASCII scene
      const canvAscii = new CanvAscii(
        {
          text,
          asciiFontSize,
          textFontSize,
      ...
      canvAscii.load();
      asciiRef.current = canvAscii; // Store the instance

      // Setup ResizeObserver after initialization
      ...
      // Return a cleanup function for the ResizeObserver
      return () => ro.disconnect();
    };

    let resizeObserverCleanup = () => {};
    init().then((cleanup) => {
      if (cleanup) {
